...
<ul style="padding-left:0px; list-style:none" class="nav-list">
  ...
  <!----=========================================---->
	...
	<?php if($this->Session->check('Auth.User') == 1){ ?>	
		<button onclick="subscribe('<?php echo $follower['User']['username']; ?>',user_auth,<?php echo $follower['User']['id']; ?>);" rel="tooltip" data-placement="top" data-original-title="Follow this Channel" data-box="close" style="opacity:1;" data-hide="fadeOut" class="close next-button"><a class="btn btn-danger"><i class="elusive-plus-sign"></i> follow</a></button> 
    <?php }else{ ?>
