<?php
if ($followers == NULL) {
    echo "<div class='elusive-plus-sign color-green media well shadow span12' style='background-color:white;'> No Followers Yet. Be the first to follow.</div>";
}
        ...
        <div class="navbar"><div class="navbar-inner"  style="padding:5px 15px 15px 5px;">
                <div class="span4" style="margin:5px 20px 5px 5px;">
                    <a  href="<?php echo $profileurl ?>" >
                        ...
                        <?php if ($followstatus != 1) { ?><a id="follow<?php echo $userid; ?>" class="btn btn-mini btn-success" onclick="subscribe('<?php echo $publicname ?>', user_auth,<?php echo $userid; ?>);
                                           switchfollow(<?php echo $userid; ?>);
                                           _gaq.push(['_trackEvent', 'Channel', 'Follow', '<?php echo $publicname ?>']);"><i class="elusive-plus-sign"></i> Follow</a> <a id="unfollow<?php echo $userid; ?>" style="display:none;" class="btn btn-mini helper-font-9" onclick="subscribeout('<?php echo $publicname ?>', user_auth,<?php echo $userid; ?>);
                                                   switchunfollow(<?php echo $userid; ?>);
                                                   _gaq.push(['_trackEvent', 'Channel', 'Follow', '<?php echo $publicname ?>']);"><i class="elusive-remove-circle"></i> Unfollow</a><?php } else { ?> <a id="unfollow<?php echo $userid; ?>" class="btn btn-mini helper-font-9" onclick="subscribeout('<?php echo $publicname ?>', user_auth,<?php echo $userid; ?>);
                                                           switchunfollow(<?php echo $userid; ?>);
                                                           _gaq.push(['_trackEvent', 'Channel', 'Follow', '<?php echo $publicname ?>']);"><i class="elusive-remove-circle"></i> Unfollow</a><a id="follow<?php echo $userid; ?>" style="display:none;" class="btn btn-mini btn-success" onclick="subscribe('<?php echo $publicname ?>', user_auth,<?php echo $userid; ?>);
                                                                   switchfollow(<?php echo $userid; ?>);
                                                                   _gaq.push(['_trackEvent', 'Channel', 'Follow', '<?php echo $publicname ?>']);"><i class="elusive-plus-sign"></i> Follow</a> <?php } ?> </a></div>
                </div>
