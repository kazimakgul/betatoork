...
public function seoplay($channel=NULL,$seo_url=NULL) {
	...
	public function seoplay2($channel=NULL,$seo_url=NULL) {
		...
		//$channel_id=$this->User->find('first',array('conditions'=>array('User.seo_username'=>$channel),'fields'=>array('User.id'),'contain'=>false));
		//print_r($channel_id);
		$gametest=$this->Game->find('first', array('conditions' => array('Game.seo_url'=>$seo_url),'fields'=>array('User.username,User.seo_username,Game.name,User.totalrate,Game.link,Game.starsize,Game.embed,Game.description,Game.id,Game.active,Game.picture')));
		print_r($gametest);
		//ReCoded
