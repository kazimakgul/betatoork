 ...
 */
	...
	public function isAuthorized() {
	 if($this->action=='channelwall' || $this->action=='wall3') {
	 	return true;
	 }
...
public function pushActivity($game_id=NULL,$channel_id=NULL,$notify=0,$email=0,$type=NULL,$msg_id=NULL) {
   ...
   $this->requestAction( array('controller' => 'orders', 'action' => 'Add_Credit',$type));
   }
...
*/
		...
		$noextension=rtrim($performer['User']['picture'], '.'.$this->getExtension2($performer['User']['picture']));
		$yesextension=$noextension.'_original.'.$this->getExtension2($performer['User']['picture']);
        $performer['User']['avatarurl']=$baseofs3.'/upload/users/'.$performer['User']['id'].'/'.$yesextension;
		//------/Generating User Avatar
...
//Bu fonksiyon More buttonunun islevini kontrol eder.
public function moreupdates_profile_ajax_home($profile_uid=NULL,$type=NULL) {
	   //$this->layout='ajax';
      //Import necessary files for wall script
	   App::import('Vendor', 'wallscript/config');
	   $this->set('gravatar',1);
...
//Bu fonksiyon More buttonunun islevini kontrol eder.
public function explore_more_feed($profile_uid=NULL,$type=NULL) {
	   //$this->layout='ajax';
      //Import necessary files for wall script
	   App::import('Vendor', 'wallscript/config');
	   $this->set('gravatar',1);
