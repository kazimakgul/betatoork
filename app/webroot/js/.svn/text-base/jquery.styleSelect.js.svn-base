    ...
    $.fn.styleSelect = function (options) {
        ...
        return this.each(function () {
            ...
            var mainContainer = $('<div tabindex="' + styledTabIndex + '"></div>')
					.css({ position: 'relative', 'z-index': parseInt(1000 - styledTabIndex), 'width': opts.jScrollPaneOptions.contentWidth })
            ...
            $('<div class="styleSelect_item"></div>')
				...
				.css({ 'position': 'absolute', 'z-index': '' + parseInt(500 - styledTabIndex) + '', 'top': opts.optionsTop, 'left': opts.optionsLeft, 'width': opts.jScrollPaneOptions.contentWidth + 18 })
				.hide();
            ...
            checkSelected(opts.styleClass, opts.jScrollPaneOptions.contentWidth + 18);
                ...
                if (!eitem.parents(".jspVerticalBar").attr('class')) {
                    ...
                    $(this).find('.styleSelect_item').slideToggle(opts.speed, function () {
                        ...
                        if ($(this).css('display') != 'none' && opts.jScrollPane == 1) {
                            ...
                            //Use jScrollPane
                            $(this).find(".styleSelect_item_content").jScrollPane(opts.jScrollPaneOptions.contentWidth + 18);
            ...
            $('#' + selectId + ' li').click(function () {
                ...
                doSelection($(this));
                if ($(this).parents('.selectDark').attr('id') == 'selectbox_city') {
                    $.post("getPrice.ashx", { 'city': $('#city').val(), 'date': $('#date').val() },
                            function (data) {
                                $('#fd').animate({ "top": 9 * -25 }, 200, function () { $('#fd').animate({ "top": parseInt(data.fd) * -25 }, 200); });
                                $('#sd').animate({ "top": 9 * -25 }, 200, function () { $('#sd').animate({ "top": parseInt(data.sd) * -25 }, 200); });
                                $('#td').animate({ "top": 9 * -25 }, 200, function () { $('#td').animate({ "top": parseInt(data.td) * -25 }, 200); });
                            }, "json");
                }
                if ($(this).parents('.selectDark').attr('id') == 'selectbox_date') {
                    $.post("getPrice.ashx", { 'city': $('#city').val(), 'date': $('#date').val() },
                        function (data) {
                            $('#fd').animate({ "top": 9 * -25 }, 200, function () { $('#fd').animate({ "top": parseInt(data.fd) * -25 }, 200); });
                            $('#sd').animate({ "top": 9 * -25 }, 200, function () { $('#sd').animate({ "top": parseInt(data.sd) * -25 }, 200); });
                ...
                if (($(this).parents('.selectDark').attr('id') == 'selectbox_cityone') || ($(this).parents('.selectDark').attr('id') == 'selectbox_citytwo')) {
                    $.post("getCityDistance.ashx", { 'cityOne': $('#dcity1').val(), 'cityTwo': $('#dcity2').val() },
                        function (data) {
                            $('#mesafe').fadeTo('fast', 0.01, function () {
                                $('#mesafe').html(data.d);
                                $('#mesafe').fadeTo('fast', 1);
                            });
                            //alert(data.d);
                        }, "json");
                }
                if ($(this).parents('.selectDark').attr('id') == 'selectbox_cityb') {
                    $('#bayi').load('scroll.aspx', { 'c': $('#cityb').val() }, function () {
